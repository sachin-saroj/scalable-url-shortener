...
from sqlalchemy import text, event, BigInteger
...
if database_url.startswith("postgresql"):
    try:
        ...
        print(f"\n[INFO] PostgreSQL probe failed ({e}). Falling back to SQLite in-memory database for testing.")
...
from app.dependencies import get_db, get_redis, get_cache
...
async def test_redis():
    ...
    real_redis = Redis(host=redis_host, port=redis_port, decode_responses=True, socket_connect_timeout=2)
    try:
