...
class TestAuthService:
    ...
    async def test_register_duplicate_username(self, db_session):
        service = AuthService(db_session)
        request1 = UserRegisterRequest(
            email="user1@example.com", username="duplicate_user", password="Password123"
        )
        await service.register(request1)

        request2 = UserRegisterRequest(
            email="user2@example.com", username="duplicate_user", password="Password123"
        )
    ...
    async def test_login_success(self, db_session):
        ...
        # Attempt login
        login_req = UserLoginRequest(email="login@example.com", password="MyPassword123")
    ...
    async def test_login_invalid_email(self, db_session):
        service = AuthService(db_session)
        login_req = UserLoginRequest(email="nonexistent@example.com", password="Password123")
    ...
    async def test_login_invalid_password(self, db_session):
        service = AuthService(db_session)
        reg_req = UserRegisterRequest(
            email="login_fail@example.com", username="loginfail", password="CorrectPassword123"
        )
        await service.register(reg_req)

        login_req = UserLoginRequest(email="login_fail@example.com", password="WrongPassword123")
        with pytest.raises(ValueError, match="Invalid email or password"):
            await service.login(login_req)
    ...
    async def test_login_deactivated_account(self, db_session):
        service = AuthService(db_session)
        reg_req = UserRegisterRequest(
            email="deactivated@example.com", username="deactivateduser", password="Password123"
        )
        await service.register(reg_req)
        ...
        login_req = UserLoginRequest(email="deactivated@example.com", password="Password123")
    ...
    async def test_refresh_tokens_invalid_type(self, db_session):
        service = AuthService(db_session)
        reg_req = UserRegisterRequest(
            email="refresh_invalid@example.com", username="refreshinvalid", password="Password123"
        )
        _, token_resp = await service.register(reg_req)
    ...
    async def test_refresh_tokens_deactivated_user(self, db_session):
        service = AuthService(db_session)
        reg_req = UserRegisterRequest(
            email="refresh_deact@example.com", username="refreshdeact", password="Password123"
        )
        _, token_resp = await service.register(reg_req)
    ...
    async def test_get_current_user_success(self, db_session):
        service = AuthService(db_session)
        reg_req = UserRegisterRequest(
            email="currentuser@example.com", username="currentuser", password="Password123"
        )
        _, token_resp = await service.register(reg_req)
