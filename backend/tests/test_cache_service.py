...
──────────────────────
Unit tests verifying the Redis cache interface, key setting, deletion,
unique click tracking, and error resilience.
"""
...
class TestCacheService:
    ...
    async def test_track_unique_click(self, cache_service):
        ...
        assert (
            await cache_service.track_unique_click("code1", "192.168.1.10", "2026-06-08") is False
        )
    ...
    async def test_graceful_degradation_on_redis_errors(self, test_redis):
        ...
        with (
            patch.object(test_redis, "get", side_effect=Exception("Redis connection error")),
