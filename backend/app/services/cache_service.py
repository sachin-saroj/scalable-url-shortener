...
class CacheService:
    ...
    async def track_unique_click(self, short_code: str, ip: str, date_str: str) -> bool:
        ...
        try:
            ...
            result = await self.redis.sadd(key, ip)
    ...
    async def get_unique_count(self, short_code: str, date_str: str) -> int:
        ...
        try:
            return await self.redis.scard(f"unique:{short_code}:{date_str}")
        except Exception as e:
    ...
    async def get_analytics(self, short_code: str) -> dict | None:
        """Get cached analytics response."""
        try:
            result = await self.redis.get(f"analytics:{short_code}")
            if result:
    ...
    async def set_analytics(self, short_code: str, data: dict, ttl: int = 60) -> bool:
